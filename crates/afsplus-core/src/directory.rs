...
pub fn encode_entry(entry: &DirEntry) -> Result<(Vec<u8>, Vec<u8>), CoreError> {
    validate_logical_entry(entry)?;
...
pub fn lookup_entry<D: BlockDevice>(
    ...
    value.map(|value| decode_entry(key, &value)).transpose()
}
        ...
        |lba, node| {
            ...
            if node.is_leaf() {
                for item in &node.items {
                    entries.push(decode_entry(&item.key, &item.value)?);
                }
            }
            Ok(())
        },
...
fn validate_logical_entry(entry: &DirEntry) -> Result<(), CoreError> {
...
mod tests {
    ...
    fn typed_directory_crosses_leaf_and_internal_boundaries() {
        ...
        let loaded = load_all(&mut dev, &geo, mutation.root_lba, 1, 2).unwrap();
        assert_eq!(loaded.entries, entries);
        assert!(loaded.summary.nodes > 1);
        assert_eq!(
            lookup_entry(&mut dev, &geo, mutation.root_lba, 1, 2, b"file-0999",)
                .unwrap()
                .unwrap()
                .child_id,
            1099
