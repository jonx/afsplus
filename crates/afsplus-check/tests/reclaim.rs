...
fn tiny_caps() -> ReclaimCaps {
    ReclaimCaps { inline_entries: 4, segment_refs: 3, table_refs: 8 }
}
...
fn tiny_caps_seal_segments_and_tables_and_drain_to_steady_state() {
    ...
    for i in 0..24 {
        vol.create_file_in_root(&format!("f{i}"), &[i as u8; 100], ts(i)).unwrap();
    }
    ...
    for i in 0..24 {
        vol.delete_file_in_root(&format!("f{i}"), ts(100 + i)).unwrap();
    ...
    assert!(steps > 5, "draining a large backlog must take multiple bounded steps");
...
fn reclaim_survives_remount_and_resumes_from_the_cursor() {
    ...
    vol.create_file_in_root("big", &[0xC3u8; 3 * BS], ts(1)).unwrap();
...
fn run_crash_matrix(
    ...
    for crash_point in 0..=log.len() {
        for state in crash_states(base, log, crash_point) {
            ...
            assert!(report.is_clean(), "{what}: checker findings {:?}", report.errors);
    ...
    assert!(pre > 0 && post > 0, "{context}: matrix must produce both outcomes");
}
...
fn crash_matrix_over_a_sealing_transaction() {
    ...
    let base = {
        ...
        vol.set_reclaim_batch_blocks(1);
        for i in 0..3 {
            vol.create_file_in_root(&format!("seed{i}"), b"seed", ts(i)).unwrap();
        }
        vol.into_device()
    };
    ...
    vol.set_reclaim_batch_blocks(1);
    vol.create_file_in_root("sealer", b"payload", ts(50)).unwrap();
    assert!(
        vol.last_commit_stats().unwrap().alloc.reclaim.segments_sealed > 0,
...
fn crash_matrix_over_segment_consumption_and_disappearance() {
    ...
    let base = {
        ...
        vol.set_reclaim_batch_blocks(1);
        for i in 0..6 {
            vol.create_file_in_root(&format!("g{i}"), b"x", ts(i)).unwrap();
        }
        vol.into_device()
    };
    let pre_generation = mount(base.clone()).unwrap().generation();
...
fn crash_matrix_over_a_mid_run_cursor_advance() {
    ...
    let base = {
        ...
        let mut vol = mount(dev).unwrap();
        vol.set_reclaim_batch_blocks(1);
        vol.create_file_in_root("big", &[0xB7u8; 3 * BS], ts(1)).unwrap();
        vol.delete_file_in_root("big", ts(2)).unwrap();
        vol.into_device()
    };
    let pre_generation = mount(base.clone()).unwrap().generation();
    ...
    run_crash_matrix(&base, &log, pre_generation, "mid-run cursor", |what, vol| {
        ...
        assert_eq!(vol.reclaim_pending_blocks(), 1, "{what}: drain after recovery");
    });
}
...
fn large_cow_rewrite_exceeds_the_legacy_253_entry_limit() {
    ...
    let steps = drain(&mut vol);
    assert!(steps >= 5, "600 blocks at 128 per step need multiple bounded steps");
    assert_eq!(vol.reclaim_pending_blocks(), 1);
    let mut dev = vol.into_device();
...
fn massive_truncate_and_bulk_unlink_quarantine_and_drain() {
    ...
    vol.set_reclaim_batch_blocks(1);
    let initial_free = vol.free_blocks();
    let a = vol.create_file_in_root("a", &vec![0xAAu8; 400 * BS], ts(1)).unwrap();
    vol.create_file_in_root("b", &vec![0xBBu8; 400 * BS], ts(2)).unwrap();
...
fn millions_of_blocks_drain_in_bounded_steps() {
    ...
    vol.preallocate_file(id, 0, prealloc_blocks * BS as u64, ts(2)).unwrap();
    ...
    assert!(steps >= 19, "1.9M blocks at 100k per step need many steps, got {steps}");
    assert_eq!(vol.reclaim_pending_blocks(), 1);
    let mut dev = vol.into_device();
