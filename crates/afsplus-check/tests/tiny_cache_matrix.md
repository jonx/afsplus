...
| `deep_split` | P | The complete 600-entry directory listing compared as one enumeration with the eight subjects and their bytes, and the unchanged root; a measured descent depth of four, two leaf splits and zero root splits; 62, 34, 32 and 32 writes with 35, 4, 0 and 0 spill writes and peaks of 2, 4, 6 and 6; 65, 37, 35 and 35 faults with 63, 35, 33 and 33 same-handle retries; sampled campaigns of 315 images at two pages (63 prefixes, 186 tears, 2 exhaustive subsets, 64 sampled subsets, outcomes 311/4), 4,267 at four pages (35 prefixes, 102 tears, 4,098 exhaustive subsets, 32 sampled subsets, outcomes 4,263/4) and 419 at eight and unlimited pages (33 prefixes, 96 tears, 258 exhaustive subsets, 32 sampled subsets, outcomes 415/4) | Seed `0x5eedba7c1003`, sample 32 |
| `deep_cross_rename` | P | Both 600-entry listings as full enumerations, the moved bytes, a link count of one and no stale entry; a measured descent depth of four; 27, 24, 24 and 24 writes with 11, 1, 0 and 0 spill writes and peaks of 2, 4, 5 and 5; 29 faults with 27 same-handle retries at two pages and 26 faults with 24 retries elsewhere; sampled campaigns of 143 images at two pages (28 prefixes, 81 tears, 2 exhaustive subsets, 32 sampled subsets, outcomes 139/4) and 131 elsewhere (25 prefixes, 72 tears, 2 exhaustive subsets, 32 sampled subsets, outcomes 127/4) | Seed `0x5eedba7c1004`, sample 32 |
...
because the four staged nodes it needs at 120 records fit the four-page profile.
...
| Spilled reservation initialization, spill evidence and faults | 100 bytes at offset 7 of the middle reservation, which the initialization clears to written; demand 3, 11, 11 and 11 with 1, 13, 5 and 0 spill writes; 12, 29, 29 and 29 writes with 9-, 14-, 22- and 27-write tails | 15, 32, 32 and 32 faults; 13, 30, 30 and 30 same-handle retries | Reservation trees of 120 records stage four nodes and trees of 160 records five, so `reservation_initialization_stages_four_nodes_at_120_records_and_five_at_160` records that measured limit |
| Spilled reservation initialization, sampled cut campaign | Seed `0xc00d17e002`, sample 64: 518, 2, 66 and 4 exhaustive subsets and 0, 128, 64 and 64 sampled subsets | 567, 247, 247 and 185 images | As above |
| Bounded write over eight extent leaves | One leaf holds 100 extent records, so the window covers 1,600 logical blocks of an 1,100-record tree and publishes one 1,600-block extent with the sparse tail behind it; demand 23 with 23, 21, 17 and 0 spill writes; 1,634, 1,634, 1,634 and 1,632 writes with 1,623-, 1,621-, 1,617- and 1,600-write tails | Four recordings through `eviction_recorded` | Nine staged nodes need a window of at least six leaves, so the fault matrix and every cut campaign of this transaction fall outside the per-test time budget |
...
| Spilled shrink and bounded shrink, spill evidence and faults | Half the tree with a nine-byte tail: the truncated byte literal, the truncated enumeration and the retained full captured view; demand 3, 7, 14 and 14 for both, with 3, 3, 6 and 0 spill writes; the shrink records 14, 16, 28 and 28 writes with 9-, 11-, 20- and 26-write tails and the bounded shrink 14, 16, 24 and 24 with 9-, 11-, 16- and 22-write tails | Shrink 17, 19, 31 and 31 faults with 15, 17, 29 and 29 retries; bounded shrink 17, 19, 27 and 27 faults with 15, 17, 25 and 25 retries | Memory device |
| Spilled shrink and bounded shrink, sampled cut campaigns | Seeds `0xc00d17e004` and `0xc00d17e005`, sample 64 | Shrink 587, 2,131, 307 and 181 images; bounded shrink 587, 2,131, 291 and 165 images | As the full-COW campaign |
| Sparse growth, exhaustive cuts and faults | Growth to four times the size with the enumeration held and the tail zeros literal; demand 3, 5, 5 and 5 with 1, 1, 0 and 0 spill writes; 10, 12, 12 and 12 writes with 9-, 11-, 11- and 11-write tails | 1,165, 4,300, 4,300 and 4,300 cut images with a 12-write budget; 12, 14, 14 and 14 faults with 10, 12, 12 and 12 retries | The staged demand of 3, 5 and 5 at 40, 120 and 500 records stays at five or below, so the eight-page and unlimited profiles report zero spills |
...
## Residual deferred window and replay combinations

[family_matrix_deferred_residuals.rs](family_matrix_deferred_residuals.rs)
closes the deferred and replay rows of the baseline matrix at 2/4/8/unlimited;
52 tests pass. The eviction fixtures format 4,096 blocks in 16-block regions
with eight log slots and 128 root names of 240 bytes, so the recovery commit
walks a directory of more than one interior level. The refusal fixture uses
256 blocks and the publication fixture 1,024, both with eight log slots.

Two runners of [common/family_matrix.rs](common/family_matrix.rs) serve this
file. `replay_eviction` records the recovery transaction of the eviction
fixture, asserts its literal staged-node demand at the unlimited profile with
zero spills, requires spill writes from every profile below that demand, and
runs the recovery fault matrix over the spill images. `replay_logging` logs
every group on a recording backend and interrupts that logging phase at every
recorded write and flush: the acknowledged record count of the interrupted
media names the allowed state, a recovery and a second recovery reach exactly
it, and a separate instance retries the interrupted group on the same handle
and either acknowledges every group or refuses with `WindowPoisoned`. The cut
campaigns of these seven families belong to their plain, retained and
exhausted fixtures in [family_matrix_deferred.rs](family_matrix_deferred.rs)
and [family_matrix_replay.rs](family_matrix_replay.rs).

| Family and part | Oracle | Modeled count per profile | Deliberate limit |
|---|---|---|---|
| Namespace recovery commit, forced eviction | The three namespace groups over 128 long root names: exact root listing, literal payload bytes of the anchor and of each group's own entry, and the absence of every name the count does not admit | 5 log writes and 3 log flushes; 19 recovery writes, 2 recovery flushes and an 18-write tail; 21 recovery faults; three staged nodes at unlimited with zero spills, and 1 spill write with a peak of 2 at two pages | Four and eight pages exceed the three-node demand and report zero spills |
| Existing-file write recovery commit, forced eviction | The byte image after a given number of acknowledged writes, with the file's other bytes literal | 7 log writes and 6 log flushes; 16 recovery writes and a 15-write tail; 18 recovery faults; two staged nodes at unlimited with zero spills | A two-node demand fits every bounded profile, so this variant records the demand and zero spills |
| Truncate recovery commit, forced eviction | Exact bytes, including the zeros the sparse growth exposes, and the exact size | 4 log writes and 4 log flushes; 15 recovery writes and a 14-write tail; 17 recovery faults; two staged nodes at unlimited with zero spills | A two-node demand fits every bounded profile |
| Mixed window recovery commit, forced eviction | Root listing, created payload, and the exact bytes and size of the subject after each count | 6 log writes and 4 log flushes; 23 recovery writes and a 22-write tail; 25 recovery faults; three staged nodes at unlimited with zero spills, and 1 spill write with a peak of 2 at two pages | Four and eight pages exceed the three-node demand |
| Shared unlink recovery commit, forced eviction | The victim leaves the namespace and enters orphan state, the two-block run keeps three references through every image, and all three objects read their exact bytes | 1 log write and 1 log flush; 21 recovery writes and a 20-write tail; 23 recovery faults; three staged nodes at unlimited with zero spills, and 1 spill write with a peak of 2 at two pages | Four and eight pages exceed the three-node demand |
| Shared write recovery commit, forced eviction | The origin moves off the first block: one two-block two-reference run before the group and one one-block two-reference run at it, with the peer's bytes exact | 2 log writes and 2 log flushes; 15 recovery writes and a 14-write tail; 17 recovery faults; two staged nodes at unlimited with zero spills | A two-node demand fits every bounded profile |
| Replacing-rename recovery commit, forced eviction | The target name resolves to the victim before the group and to the incoming object at the group, the replaced object carries the orphan flag, and both payloads are literal | 1 log write and 1 log flush; 22 recovery writes and a 21-write tail (24 and 23 at two pages); 24 recovery faults (26 at two pages); three staged nodes at unlimited with zero spills, and 3 spill writes with a peak of 2 at two pages | Four and eight pages exceed the three-node demand |
| Resource refusal of `window_write_file_at` and `window_truncate_file` | A committed filler and one-block window creates exhaust the window allocator; a 40-block write and a partial shrink each report `NoSpace` with zero writes and zero flushes, hold the generation, the 22 staged operations and the anchor bytes, and a remount keeps exactly the one acknowledged group | 2 refusals over 21 padding creates; a corrective step of 5 commits, after which both calls succeed and recovery reads 4,107 literal bytes | One volume geometry; the refusal of a padding create is the documented `window_op` path |
| Read failures of the publication path | `window_fsync` issues no read for a namespace group or for a data group with its barrier, so no read failure reaches it; every read of `window_commit` fails in turn, the call reports the injected error, the same publication and an independent create return `WindowPoisoned` with the generation held, and a remount recovers either the two acknowledged groups alone or the complete window | 0 fsync reads; 27 commit read faults per profile, 21 before the checkpoint write and 6 after it | Memory device; a read that fails after the checkpoint write is the ambiguous publication of the same rule |
| Logging faults of the orphan delete, shared unlink, replacing rename and shared write | A fault at every logging write and flush: the interrupted media carries an acknowledged prefix, a recovery and a second recovery reach exactly it, and the same-handle retry refuses with `WindowPoisoned` | 2 faults (1 write, 1 flush) for the orphan delete, the shared unlink and the replacing rename, with acknowledged counts 1/1; 4 faults (2 writes, 2 flushes) for the shared write with counts 3/1; every case requires a remount | The data write of a logged write poisons the window, so no case admits a same-handle retry |

Every oracle constant of this file is spelled out separately from the value
the operation supplies. Negative controls: a shifted anchor payload byte, a
wrong publication patch value, a wrong retried truncate size and a wrong
namespace staged-node demand each fail their test, and the source is restored
from the commit afterwards with a matching hash.

## Residual persistent snapshot combinations

[family_matrix_snapshot_residuals.rs](family_matrix_snapshot_residuals.rs)
closes the snapshot rows of the baseline matrix at 2/4/8/unlimited; 24 tests
pass. The eviction fixtures format 4,096 blocks in 16-block regions with 128
root names of 240 bytes; the identity and ledger fixtures use 1,024 blocks in
256-block regions. Membership is read back through `snapshot_list` on the
selected checkpoint, and live and captured bytes carry a sentinel past the end.

| Family and part | Oracle | Modeled count per profile | Deliberate limit |
|---|---|---|---|
| Maintenance step, forced eviction | Over 128 long root names: registry membership, the deleted name's absence, the keeper's bytes and both captured objects | 7 writes, 2 flushes and a 6-write tail; 9 faults and 7 same-handle retries; two staged nodes at unlimited with zero spills | A two-node demand fits every bounded profile, so this variant records the demand and zero spills |
| Release of the last view, forced eviction | Membership empties, the view becomes unreachable and the keeper's bytes hold | 26 writes, 2 flushes and a 25-write tail; 28 faults and 26 same-handle retries; two staged nodes at unlimited with zero spills | A two-node demand fits every bounded profile |
| Snapshot-aware recovery commit, forced eviction | Every image reads the live bytes of its acknowledged prefix and the captured historical bytes | 4 log writes and 4 log flushes; 13 recovery writes and a 12-write tail; 15 recovery faults; two staged nodes at unlimited with zero spills | A two-node demand fits every bounded profile; the cut campaign belongs to the plain fixture of [family_matrix_snapshot.rs](family_matrix_snapshot.rs) |
| Exhausted registry identity space | The control record of the registry root carries the largest representable next identity, written into the decoded tree node and resealed with its own generation. Creation reports `Format(Overflow("snapshot ID exhausted"))` with zero writes and zero flushes, live and after a remount, while the registered view keeps its captured bytes; releasing that view frees a slot and no identity; an ordinary create publishes one checkpoint | 3 refusals per profile; the planted next identity is 18,446,744,073,709,551,615 | The checker accepts the planted image before and after every step |
| Bounded ledger scan that resumes and wraps | A ledger of 16 lifetime records over three captured one-block files between live neighbours: the first step scans the 8-record admission without wrapping and the second scans 8 and wraps, with registry membership, every deleted name's absence, the live neighbours' bytes and every captured object exact | 2 bounded steps per profile | One admission size |
| The wrapping maintenance pass through the driver | The fixture runs the first bounded step, so the recorded commit is the one that crosses the resume boundary and reports a wrapped scan | 6 writes, 2 flushes and a 5-write tail; 115 cut images (111 and 4) with a 12-write budget; 8 faults and 6 same-handle retries | One ledger geometry |

Negative controls: a wrong keeper payload, a wrong admission record count and
a wrong maintenance staged-node demand each fail their test, and the source is
restored from the commit afterwards with a matching hash.

```sh
export CARGO_HOME=$HOME/.cargo
export RUSTUP_HOME=$HOME/.rustup
export CARGO_TARGET_DIR=/private/tmp/afsplus-wt-cache-deferred-residuals/target
export CARGO_NET_OFFLINE=true
export PATH="$CARGO_HOME/bin:$PATH"
for target in family_matrix_deferred_residuals family_matrix_snapshot_residuals; do
  cargo test --offline -p afsplus-check --all-features --test "$target" -- --test-threads=2
done
for target in intent_log family_matrix_deferred family_matrix_replay shared_crash; do
  cargo test --offline -p afsplus-check --all-features --test "$target" -- --test-threads=2
done
cargo test --offline -p afsplus-core --all-features --lib
cargo fmt --all -- --check
cargo clippy --offline -p afsplus-check --all-features --tests -- -D warnings
make check-docs
```
...
| Family and part | Oracle | Modeled count per profile | Deliberate limit |
|---|---|---|---|
| Mid-run reclaim cursor, cuts and faults | 256-block image with four inline entries, three segment and eight table references; the complete root enumeration of `anchor` and `filler`, the literal anchor bytes with an EOF sentinel, free and pending accounting of (208, 18) before and (208, 20) after, and a consumed-block offset of 0 before and 2 after | 7 writes and 2 flushes with a 6-write tail; 197 cut images (193/4) with a 12-write budget; 9 faults (7 writes, 2 flushes) and 7 same-handle retries | Single-region fixture with one three-block run |
| Mid-run reclaim cursor, retained view and ambiguous publication | The snapshot taken ahead of the reclaimed blocks keeps the anchor's captured metadata and bytes through every image; both ambiguous modes poison with no I/O and remount to exactly one publication | 197 cut images, 9 faults, 7 same-handle retries, 2 ambiguous cases | Memory device |
