...
impl Checkpoint {
    ...
    pub fn decode(block: &[u8], expected_uuid: &[u8; 16]) -> Result<Checkpoint, FormatError> {
        ...
        // Nothing follows the payload (ADR-112, in the header verification):
        // a checkpoint with snapshot roots resealed with the short length
        // does not read as one without (ADR-111).
        let mut uuid = [0u8; 16];
