...
| [dev-packet-matrix.sh](dev-packet-matrix.sh) | Development check: runs the host `DosPacket` and trackdisk matrices against the AROS source headers on a host without a built SDK, and type-checks the handler shell when an SDK include tree exists; no qualification claim | [implementation/stage-c-gap.md](../implementation/stage-c-gap.md) |
