...
| [conformance.md](conformance.md) | Versioned reference images, official CLI output and independent Rust↔C read/write slices decode to their expected result | `cargo test -p afsplus-tools --all-features`; portable-C gate; checker reader tests and dedicated corruption corpus | M00, M02, M05, M12 |
...
| [allocation-qualification.md](allocation-qualification.md) | Q3 low-space progress, exact ENOSPC, maximum-region amplification and multi-node allocation-root pressure | `cargo test -p afsplus-check --test allocation_pressure`; optimized ignored evidence workloads | M03, M14 |
| [orphan-qualification.md](orphan-qualification.md) | Bounded open-unlinked identity, restartable cleanup, corruption rejection and logged final-delete/replacement replay | `cargo test -p afsplus-vfs --test api`; `cargo test -p afsplus-check --test orphans --test intent_replay_orphans`; `make portable-c-gate` | M03, M04, M05, M14 |
| [symlink-qualification.md](symlink-qualification.md) | Inline target codec parity, exact adapter round-trip, corruption rejection and namespace crash atomicity | dedicated Rust format/VFS/checker tests; `make portable-c-gate` | M03, M05, M12, M14 |
...
| [mounted-volume-testing.md](mounted-volume-testing.md) | What a person can do with a volume mounted through macFUSE, and how to test a mounted driver without leaving the desktop unable to start | `tools/check-mounted-usage.sh` | M08 |
| [image-workflow.md](image-workflow.md) | A developer's image workflow on the host: create a sparse image, fork it by file clone, mount the fork, check, diff against the base, replay a session on another fork | `tools/check-image-workflow.sh` (`make image-workflow`) | M08 |
| [fuzzing.md](fuzzing.md) | Every independent metadata parser survives mutated input | `make rust-codec-fuzz-gate` and `make portable-c-fuzz-gate`; exact replay artifacts on failure | M01 |
