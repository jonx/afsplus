...
| AROS handler ([\[M06\]](implementation/milestones.md)) | Runs on real AROS: a hosted `darwin-aarch64` build boots it, mounts a volume, passes the DOS semantics and device write-ordering gates and holds one instance per medium, at C boundary interface revision 15; QEMU, m68k and Apple hardware open |
