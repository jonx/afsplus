...
3. Remain easy to implement elsewhere: a portable core, a tiny reader profile, public format documentation, fixed identification records, feature negotiation, conformance images, FUSE support, and no AROS path syntax in the on-disk format.
...
See `docs/22-pfs3-and-pfs4-lessons.md` and `adr/ADR-019-pfs3-design-reference.md`.

## Newly identified design choices

Several design decisions are included specifically to reduce future maintenance cost and resource use:
...
A normal read-only mount can still modify media in some filesystem designs, for example during journal recovery or housekeeping. AFS+ defines a strict `NO_CHANGES` mount mode in which the implementation must not write a single block. This is intended for recovery, debugging, forensic access, and compatibility testing.

### Shared repair core

Filesystem repair logic must not become a second independent implementation of the format. `afsplus-check`, the AROS handler, the FUSE implementation, and host-side validation all use the same portable parsing and invariant code wherever possible.

### Rebuildable accelerators

The global catalog and change stream improve performance but are not authoritative filesystem structures. A volume remains correct if they are disabled, unsupported, stale, or rebuilt.

### Optional inline data

Tiny files dominate source trees, Cargo metadata, package caches, editor state, and configuration directories. A future optional `inline-data` feature can store very small file payloads with object metadata, reducing allocation overhead and I/O. It is deliberately not required by the base format.

## Repository map

- `docs/` - normative architecture and behavior specifications
- `adr/` - architecture decision records
- `spec/` - machine-oriented constants and format definitions
- `api/` - draft public APIs
- `implementation/` - phased implementation plan
- `testing/` - conformance, fuzzing, crash testing, and benchmarks
...
This is a development specification. Fields marked `TBD` are not frozen. Once implementation begins, incompatible format changes must update the format epoch or be represented through feature negotiation as defined in the compatibility specification.
