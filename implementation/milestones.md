...
| I/O and write-amplification accounting | Trace records successful block reads, writes, bytes and flushes | Implemented counters, [per-command CPU/RSS collection](../testing/benchmark-contract.md#per-command-host-accounting) and [phased requested-heap/I/O workload](../testing/benchmark-contract.md#phased-requested-heap-workload); per-variant attribution, individual cache ownership and steady RSS remain open. |
...
| Tiny-cache matrix and fuzz/property tests | [Tree spill/reload tests](../crates/afsplus-core/src/cow_tree.rs) exercise 2/4/8 staged pages; [volume mutations](../crates/afsplus-core/src/volume.rs) use the default unbounded entry point | Partial: integrate cache policy across publication paths, account for spilled nodes in allocation/cache bookkeeping and commit metrics, then qualify exact states and failure artifacts per profile. Preserve separate fuzz/property gates. |
...
| roadmap-31 | tiny-cache test matrix | [Stage A: make the core executable](../ROADMAP.md#stage-a-make-the-core-executable) | Partial | [evidence](../testing/developer-harness.md) |
