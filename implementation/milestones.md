...
| \[M06\] | AROS handler | Partial — Hosted: S0 to S3, DOS gate, driver probe, bench 3.1 s (FFS 1.4) at revision 17; QEMU/m68k S0; S2/S3 on QEMU and Native, hardware, budget, A500 open | classic apps operate without recompilation | [aros-native-bridge](../docs/aros-native-bridge.md), [14](../docs/14-paths-and-namespaces.md) | [aros-system-volume-qualification](../testing/aros-system-volume-qualification.md) | [\[Stage C\]](../ROADMAP.md#stage-c-integrate-aros-and-begin-independent-c-portability) |
| \[M07\] | FS API v2 | Portable subset — handles/fsync/reflinks, backup/restore, C mutation subset, AROS v2 groups and transport run on Hosted; a consumer and native open | 64-bit and capability tests pass | [13](../docs/13-filesystem-api-v2.md) | [test-strategy](../testing/test-strategy.md), [security/restore](../testing/security-model-conformance.md), [backup archive](../testing/backup-archive-qualification.md) | [\[Stage C\]](../ROADMAP.md#stage-c-integrate-aros-and-begin-independent-c-portability) |
...
- [Stage B acceptance inventory](#stage-b-acceptance-inventory)
...
| Stage B | b-allocation | Epoch-1 allocation state selected, implemented and qualified on the host (B1) | Complete | [ADR-067](../adr/ADR-067-epoch1-allocation-state.md), [allocation qualification](../testing/allocation-qualification.md): [ENOSPC without I/O, headroom, near-full power cuts, 144 dirty regions](../crates/afsplus-check/tests/allocation_pressure.rs), [allocation-root crash matrix](../crates/afsplus-check/tests/alloc_crash.rs) |
| Stage B | b-data-policy | Full COW by default, persistent private in-place opt-in, COW for shared and snapshot-protected ranges (B2) | Complete | [ADR-062](../adr/ADR-062-explicit-hybrid-data-updates.md), [ADR-065](../adr/ADR-065-persistent-data-update-policy.md), [data-policy qualification](../testing/data-policy-qualification.md): [policy workloads](../crates/afsplus-check/tests/data_policy.rs), [persistence, eligibility and power cuts](../crates/afsplus-check/tests/data_policy_persistence.rs); both readers refuse the flag without its feature |
| Stage B | b-durability | Checkpoint COW, bounded group commit and the intent log for namespace and existing-file updates, with interrupted replay (B3) | Complete | [ADR-063](../adr/ADR-063-intent-log-epoch1.md), [ADR-064](../adr/ADR-064-intent-log-data-update-compatibility.md), [write/truncate qualification](../testing/intent-log-write-truncate-qualification.md): [intent log](../crates/afsplus-check/tests/intent_log.rs), [shared-range crashes](../crates/afsplus-check/tests/shared_crash.rs), [fsync workloads](../crates/afsplus-check/tests/fsync_workloads.rs) |
| Stage B | b-structures | Every structure of B4 except the two tooling lines below | Complete | [Roadmap entries 21 to 27, 34 and 35](#individual-list-item-completion), each Complete with its test |
| Stage B | b-security | Security preservation container and the metadata that travels with an object (B5) | Complete | [ADR-100](../adr/ADR-100-exact-object-record-admission.md), [ADR-101](../adr/ADR-101-security-preservation-container.md), [ADR-102](../adr/ADR-102-clone-metadata-inheritance.md), [ADR-105](../adr/ADR-105-security-reference-admission.md), [ADR-109](../adr/ADR-109-owned-chains-under-snapshots.md): [container, projection policy and crash matrix](../crates/afsplus-check/tests/security_container.rs), [clone metadata](../crates/afsplus-check/tests/clone_metadata.rs), [chains under snapshots](../crates/afsplus-check/tests/chain_snapshots.rs) |
| Stage B | b-c-parity | Every wire layout the Rust codec states has a second statement in portable C | Complete | [Q15 closed](open-questions.md): cross-read tests for [object admission and the container](../crates/afsplus-format/tests/security_c.rs), [attributes](../crates/afsplus-format/tests/attributes_c.rs), [reclaim blocks](../crates/afsplus-format/tests/reclaim_c.rs), [checkpoint](../crates/afsplus-format/tests/checkpoint_c.rs), [snapshot records](../crates/afsplus-format/tests/snapshot_c.rs), [pinned constants](../crates/afsplus-format/tests/c_constants.rs), [zero tail on the lookup path](../crates/afsplus-check/tests/zero_tail.rs) |
| Stage B | b-explain | Explain APIs and command (roadmap-36) | Complete | [block, object and path](../crates/afsplus-check/tests/explain.rs), [extent, checkpoint, reclaim, space and feature](../crates/afsplus-check/tests/explain_more.rs), [the command](../crates/afsplus-tools/tests/explain_cli.rs) |
| Stage B | b-diff | Semantic image diff and command (roadmap-37) | Complete | [image pairs against literal expectations](../crates/afsplus-check/tests/image_diff.rs), [the command](../crates/afsplus-tools/tests/image_diff_cli.rs) |

## Stage B acceptance inventory

Stage B resolves the epoch-1 architecture blockers: each blocker has a
decision on record and an executable proof on the host. It does not qualify
hardware, freeze the wire or integrate a platform; those sentences of the
roadmap's B sections name work that later stages own, listed here so that no
gate above is read as covering it.

| Not in Stage B | Where the roadmap says it | Owner |
|---|---|---|
| Exact byte offsets, the wire epoch and the freeze of every layout. The reserved FIELDS of every structure are decided ([ADR-110](../adr/ADR-110-exact-reclaim-admission.md) to [ADR-114](../adr/ADR-114-reserved-header-fields.md)); the offsets themselves are not | B1, B3, B5 ("subject to M14 review") | M14, [\[Stage F\]](../ROADMAP.md#stage-f-production-qualification) |
| Real-device flush barriers and hardware acceptance | B3 | M14, Stage F |
| Portable C allocation and data emission; the C writer appends namespace and data-free records only | B2, B3 ([remaining boundary](../testing/intent-log-write-truncate-qualification.md#remaining-boundary)) | M14 |
...
required coverage: 18 codec targets, caller admission and Unicode properties, and nine generated families with retained artifacts. Multi-step fragmented orphan cleanup, per-file policy placement, power cuts and injected I/O errors, and intermediate reads keep their named owners in [fuzzing](../testing/fuzzing.md#generated-operation-families). Frozen-field decisions, portable C qualification and future
...
The three admission groups identified by this review have executable
...
| roadmap-36 | explain APIs | [Stage B / B4. Core filesystem structures](../ROADMAP.md#b4-core-filesystem-structures) | Complete | [block, object and path](../crates/afsplus-check/tests/explain.rs), [extent, checkpoint, reclaim, space and feature](../crates/afsplus-check/tests/explain_more.rs), [the command](../crates/afsplus-tools/tests/explain_cli.rs) |
...
| roadmap-38 | AROS handler | [Stage C: integrate AROS and begin independent C portability](../ROADMAP.md#stage-c-integrate-aros-and-begin-independent-c-portability) | Partial | [S0 to S3 on Hosted](../testing/aros-system-volume-qualification.md), [S2](../tools/check-hosted-aros-s2.sh), [S3](../tools/check-hosted-aros-s3.sh); QEMU and Native S2/S3 open ([C14](stage-c-gap.md#c14-aros-handler-qualification-ladder)) |
| roadmap-39 | DOS compatibility | [Stage C: integrate AROS and begin independent C portability](../ROADMAP.md#stage-c-integrate-aros-and-begin-independent-c-portability) | Partial | [DOS gate on Hosted](../tools/check-hosted-aros-dos.sh), [adapter matrix](../crates/afsplus-aros/tests/dos_compat.rs); `ACTION_FORMAT` and the emulator runs open ([C2](stage-c-gap.md#c2-dos-compatibility)) |
| roadmap-40 | Filesystem API v2 | [Stage C: integrate AROS and begin independent C portability](../ROADMAP.md#stage-c-integrate-aros-and-begin-independent-c-portability) | Partial | [entry groups](../crates/afsplus-aros/tests/api_v2.rs), [transport on Hosted](../tools/check-hosted-aros-dos.sh); a consumer open ([C4](stage-c-gap.md#c4-filesystem-api-v2-and-the-modern-64-bit-api)) |
| roadmap-41 | modern 64-bit API | [Stage C: integrate AROS and begin independent C portability](../ROADMAP.md#stage-c-integrate-aros-and-begin-independent-c-portability) | Partial | [positioned 64-bit I/O and object IDs](../crates/afsplus-aros/tests/object_ids.rs); consumer open ([C4](stage-c-gap.md#c4-filesystem-api-v2-and-the-modern-64-bit-api)) |
| roadmap-42 | clone/reflink capability API | [Stage C: integrate AROS and begin independent C portability](../ROADMAP.md#stage-c-integrate-aros-and-begin-independent-c-portability) | Complete | [clone entry points](../crates/afsplus-aros/tests/api_v2.rs), [`AFSPlusClone`](../native/aros/tools/afsplus_clone.c) and the tour on Hosted ([C5](stage-c-gap.md#c5-clone-and-reflink-capability-api)) |
| roadmap-43 | access-intent/preallocation mapping | [Stage C: integrate AROS and begin independent C portability](../ROADMAP.md#stage-c-integrate-aros-and-begin-independent-c-portability) | Partial | [preallocate and advise](../crates/afsplus-aros/tests/api_v2.rs); hints with an effect open ([C6](stage-c-gap.md#c6-access-intent-and-preallocation-mapping)) |
| roadmap-44 | mmap-friendly large-file path | [Stage C: integrate AROS and begin independent C portability](../ROADMAP.md#stage-c-integrate-aros-and-begin-independent-c-portability) | Partial | [extent map](../crates/afsplus-vfs/tests/extent_map.rs); the multi-block path open ([C7](stage-c-gap.md#c7-mmap-friendly-large-file-path)) |
| roadmap-45 | notifications | [Stage C: integrate AROS and begin independent C portability](../ROADMAP.md#stage-c-integrate-aros-and-begin-independent-c-portability) | Complete | [watch table](../crates/afsplus-aros/tests/notifications.rs), NotifyRequest and v2 watch [on Hosted](../tools/check-hosted-aros-dos.sh) ([C8](stage-c-gap.md#c8-notifications)) |
| roadmap-46 | health reporting | [Stage C: integrate AROS and begin independent C portability](../ROADMAP.md#stage-c-integrate-aros-and-begin-independent-c-portability) | Complete | [health log](../crates/afsplus-aros/tests/health.rs), [events 5 to 7](../crates/afsplus-aros/tests/health_events.rs), `HEALTH_EVENTS` over the transport ([C9](stage-c-gap.md#c9-health-reporting)) |
| roadmap-47 | trace streaming / developer attachment | [Stage C: integrate AROS and begin independent C portability](../ROADMAP.md#stage-c-integrate-aros-and-begin-independent-c-portability) | Complete | [trace ring drained on Hosted](../docs/aros-native-bridge.md), [stable codes](../api/debug_observability.h) ([C10](stage-c-gap.md#c10-trace-streaming-and-developer-attachment)); a serial front-end for the emulators is a Stage D concern |
| roadmap-48 | structured management APIs | [Stage C: integrate AROS and begin independent C portability](../ROADMAP.md#stage-c-integrate-aros-and-begin-independent-c-portability) | Partial | [info JSON](../crates/afsplus-aros/tests/management.rs), [`AFSPlusInfo`](../native/aros/tools/afsplus_info.c); error documents and dry runs open ([C11](stage-c-gap.md#c11-structured-management-apis)) |
| roadmap-49 | Rust/C integration boundary | [Stage C: integrate AROS and begin independent C portability](../ROADMAP.md#stage-c-integrate-aros-and-begin-independent-c-portability) | Partial | [boundary tests](../crates/afsplus-aros-ffi/tests/interface.rs), [link check](../tools/check-aros-ffi.sh); emulator reruns and structured results open ([C1](stage-c-gap.md#c1-rustc-integration-boundary)) |
| roadmap-50 | generic file-backed virtual block device for mounting images | [Stage C: integrate AROS and begin independent C portability](../ROADMAP.md#stage-c-integrate-aros-and-begin-independent-c-portability) | Partial | [block-device contract](../docs/aros-block-device-contract.md), [`AFSPlusDriverProbe`](../native/aros/tools/afsplus_driver_probe.c), [fdsk ordering gate](../tools/check-aros-fdsk-ordering.sh); run-time attach open ([C12](stage-c-gap.md#c12-file-backed-virtual-block-device)) |
| roadmap-51 | native AROS benchmark runner | [Stage C: integrate AROS and begin independent C portability](../ROADMAP.md#stage-c-integrate-aros-and-begin-independent-c-portability) | Complete | [runner](../tools/bench-hosted-aros.sh), [contract](../testing/benchmark-contract.md#native-aros-runner) ([C13](stage-c-gap.md#c13-native-aros-benchmark-runner)); emulator and hardware numbers are Stage F |
| roadmap-52 | classic/single-user security preservation adapter | [Stage C: integrate AROS and begin independent C portability](../ROADMAP.md#stage-c-integrate-aros-and-begin-independent-c-portability) | Complete | [adapter](../crates/afsplus-aros/tests/classic_security.rs), `SECURITY=` in the DOSDriver Control ([C3](stage-c-gap.md#c3-classic-single-user-security-preservation-adapter)) |
| roadmap-53 | third-party probe kit | [Stage D: portability and host tooling](../ROADMAP.md#stage-d-portability-and-host-tooling) | Complete | [the probe](../portable/probe/afsplus_probe.c), [the gate and its vectors](../tools/check-probe-kit.sh), [docs/18](../docs/18-third-party-integration.md) |
| roadmap-54 | JSON/structured tooling schemas | [Stage D: portability and host tooling](../ROADMAP.md#stage-d-portability-and-host-tooling) | Complete | [the schema files](../spec/schemas/), [the gate](../tools/check-json-schemas.py), [tools-spec](../tools/tools-spec.md#common-command-contract) |
| roadmap-55 | sparse-image create/mount/fork/replay workflow | [Stage D: portability and host tooling](../ROADMAP.md#stage-d-portability-and-host-tooling) | Partial | [image-workflow](../testing/image-workflow.md), [the gate](../tools/check-image-workflow.sh); create, fork, mount, diff and session replay pass, a scenario bundle onto a file image is open |
| roadmap-56 | compatibility profiles | [Stage D: portability and host tooling](../ROADMAP.md#stage-d-portability-and-host-tooling) | Complete | [the five policies](../profiles/), [afsplus-info compatibility](../tools/tools-spec.md#afsplus-info), [unit and CLI tests](../crates/afsplus-tools/tests/cli.rs) |
| roadmap-57 | FUSE mmap and parallel page-fault qualification | [Stage D: portability and host tooling](../ROADMAP.md#stage-d-portability-and-host-tooling) | Complete | [the gate](../tools/check-mount-mmap.py), [mounted-volume-testing](../testing/mounted-volume-testing.md#memory-mapped-files-under-parallel-page-faults) |
