...
| Window ownership on refusal | M04; intent-window engine | [Preflight ownership/retry gate](../testing/intent-log-write-truncate-qualification.md#window-ownership-on-preflight-refusal) | Preserve logged and unlogged state across read-only admission failures; require each new window entry and diagnostic linkage to retain these ownership and retry guarantees. |
| Internal diagnostic coverage | M01/M03, Stage A; common commit engine and replay harness | [Common checkpoint-tail ring](../testing/developer-harness.md#common-checkpoint-tail-flight-recorder), bounded storage and publication-failure tests | Extend [core API spans](../testing/developer-harness.md#core-api-call-spans) and [deferred-window linkage](../testing/developer-harness.md#deferred-window-observation) with object/block/view identities, mount/recovery and platform-adapter scopes, then allocator/tree/cache events; extend the [versioned category/live replay export](../testing/developer-harness.md#selected-category-and-live-delivery-bundles) to those subsystems and additional real-consumer schedules; verify event loss and correlation under faults without changing filesystem outcomes. |
