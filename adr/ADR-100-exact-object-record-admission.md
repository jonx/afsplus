...
([ADR-065](ADR-065-persistent-data-update-policy.md)): a validated
...
M3. The portable C reader applies the same four rules through one shape
check shared by its generic and its symlink decoder. Both implementations
prove the same verdict per image
([ADR-029](ADR-029-dual-reference-implementations.md)): the resealed negative
images of the experiment are generated by
`crates/afsplus-format/tests/security_c.rs` and read by both codecs, and
`crates/afsplus-check/tests/security_c.rs` repeats the comparison through the
volume lookup on real images.
...
| Conformance image | The cross-read tests of M3 generate the resealed negative images and the canonical controls on every run; retained corpus files follow with the M14 corpus |
...
- The Rust and C readers agree on header flags, payload length and tail.
- A future extension costs an object flag and a feature identity. The
  security reference of [ADR-101](ADR-101-security-preservation-container.md) is the first
  user of that path.
