...
impl<FS: Filesystem> Session<FS> {
    ...
    /// Wrap an already mounted, message-oriented FUSE transport.
    ///
    /// The caller remains responsible for the transport's mount and unmount
    /// lifetime. The initial FUSE handshake is completed before this function
    /// returns.
    pub fn from_transport(
        ...
        let ch = Channel::from_transport(transport);
        let mut session = Session {
            filesystem: FilesystemHolder {
                fs: Some(filesystem),
            },
